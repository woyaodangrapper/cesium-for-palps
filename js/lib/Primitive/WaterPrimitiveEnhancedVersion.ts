...
export class WaterPrimitiveEnhancedVersion {
  /**
   * scene: Cesium.Scene;
  ...
  constructor(options) {
    ...
    positions.forEach((p) => {
      ...
      z += Math.sin(lat);
      this._positions.push(Cesium.Cartesian3.fromRadians(p.longitude, p.latitude, this._height));
    });
    ...
    // 初始化Uniform变量
    this._initUniforms = {
      normalMapUrl: Cesium.defaultValue(options.normalMapUrl, '/assets/images/water/normals.jpg'),
      size: Cesium.defaultValue(options.rippleSize, 50.0),
      waterColor: Cesium.defaultValue(options.waterColor, Cesium.Color.fromCssColorString('#001e0f')),
    ...
    Cesium.UniformState.prototype.updateFrustum = function (frustum) {
      ...
      Cesium.Matrix4.clone(
        Cesium.defaultValue(frustum.customProjectionMatrix, frustum.projectionMatrix),
        this._projection,
      );
  ...
  _createReflectionWaterMaterial() {
    ...
    const { normalMapUrl, size, waterColor, waterAlpha, rf0, lightDirection, sunShiny, distortionScale } =
      this._initUniforms;
  ...
  _updateVirtualCamera(camera) {
    ...
    // 获得相机指向反射面的向量
    let view = Cesium.Cartesian3.subtract(this._reflectorWorldPosition, cameraWorldPosition, new Cesium.Cartesian3());
    ...
    // 拷贝当前反射相机的投影矩阵
    const projectionMatrix = Cesium.Matrix4.clone(this._virtualCamera.frustum.projectionMatrix);
    ...
    this._virtualCamera.frustum.customProjectionMatrix = Cesium.Matrix4.clone(projectionMatrix); // TODO: 要改源码
